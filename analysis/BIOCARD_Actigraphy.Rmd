...
data_act_clean_analysis <- data_act_clean |>
  ...
  left_join(data_champs_clean[, c('SUBJECT_ID','VISITNO', "LOW_INT_FREQ", "HIGH_INT_FREQ","ALL_INT_FREQ")], 
            by = c('SUBJECT_ID','VISITNO')) |>
  mutate(LOW_INT_FREQ_z = (LOW_INT_FREQ - mean(LOW_INT_FREQ, na.rm = TRUE)) / sd(LOW_INT_FREQ, na.rm = TRUE),
...
```{r AB42AB40 plot, fig.width=14, fig.height=13}
AB42AB40_act_data_plot <- AB42AB40_act_data |> 
  mutate(CHAMPS_group_low = ifelse(LOW_INT_FREQ_z >= median(LOW_INT_FREQ_z, na.rm = TRUE), 
        "LOW_INT_FREQ >= median(LOW_INT_FREQ)", "LOW_INT_FREQ < median(LOW_INT_FREQ)"),
         CHAMPS_group_high = ifelse(HIGH_INT_FREQ_z >= median(HIGH_INT_FREQ_z, na.rm = TRUE), 
         "HIGH_INT_FREQ >= median(HIGH_INT_FREQ)", "HIGH_INT_FREQ < median(HIGH_INT_FREQ)"),
         CHAMPS_group_all = ifelse(ALL_INT_FREQ_z >= median(ALL_INT_FREQ_z, na.rm = TRUE), 
          "ALL_INT_FREQ >= median(ALL_INT_FREQ)", "ALL_INT_FREQ < median(ALL_INT_FREQ)")) |>
...
p2 <- ggplot(AB42AB40_act_data_plot, aes(x = time, y = AB42AB40_zscore)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
...
p3 <- ggplot(AB42AB40_act_data_plot, aes(x = time, y = AB42AB40_zscore)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE) +
  facet_wrap(~ CHAMPS_group_all) + 
  labs(title = "Standardized AB42AB40 Measure Over Time Since Baseline (years)",
       x = "Time (Years since Baseline)",
       y = "AB42AB40 zscore",
       color = "LTAC10 Group",
